...
A more detailed explanation regarding the code used to carry out the procedure described in this sub-heading can be found in @sec-A-obtdat.
...
Correlation is a valuable tool in stock market analysis, but it does not imply causation. Having a high or low correlation between two variables does not imply that one variable causes changes in the other. Correlation simply measures the strength and direction of the linear relationship between two variables, without considering other factors that may influence them.

As also exposed in @edwards22, the correlation is closely related to the volatility of the market and of the shares, being able to see that, during periods of greater volatility, such as the financial crisis of 2008, the shares can have a tendency to be more correlated, even if they are in different sectors. International markets can also become highly correlated during times of instability. Investors may want to include assets in their portfolios that have low market correlation to equity markets to help manage their risk.
...
As explained by @monaghan19bvsc Beta is related to mapping, but they are not the same. As explained above, correlation is a measure of how linearly related two variables are, Beta, on the other hand, is a measure of how strongly related two variables are, indicating how much one variable changes when another variable changes by one unit. Beta can be calculated from the correlation using the following equation, @eq-beta:
...
-   The first dimension is comprised of the number of samples obtained by sectioning the observations of the different series into consecutive two-dimensional two-dimensional vectors.
...
-   The third dimension is the number of series in each two-dimensional vector.
